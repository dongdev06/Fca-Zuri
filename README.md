...
Lưu Ý! Đây Là Sản Phẩm Được R1zaX Duy Trì và Phát Triển ( Fca Được Mod Lại Từ Scr Của Fca-Horizon-Remake ), Nếu Có Lỗi Hãy Thử Sử Dụng Sang Sản Phẩm Khác !
...
```bash
npm install fca-rqzax@latest
```
Hoặc
```bash
npm i fca-rqzax@latest
```
...
```javascript
const login = require("fca-rqzax"); // lấy từ lib ra 
